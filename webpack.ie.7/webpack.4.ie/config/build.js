  ...
  const chalk = require('chalk'); //棰滆壊鎻掍欢
  const webpackBuildConfig = require('./webpack.config'); //build 閰嶇疆
  ...
  (async() => {
      ...
      function downLibsAndBuild(rawData) {
          ...
          if (pages.length) {
              ...
              console.log('import sucess!npm run build...');
              webpack(webpackBuildConfig, (err, stats) => {
                  if (err) throw err;
                  //process鏄叏灞€瀵硅薄锛屾棤闇€寮曞叆--
